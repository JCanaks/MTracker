...
app.put('/requests/:requestId', verifyToken, (req, res) => {
  ...
  if (request == null || invalidProperty(request)) {
    return res.status(400).json({
      message: 'Please fill in the neccesary Fields',
    });
  }
  ...
  pool.connect((error, client, done) => {
    ...
    client.query('SELECT * from "request" where "userId" = $1 and "requestId"=$2', [req.userData.userId, req.params.requestId], (queryError, result) => {
      ...
      if (result.rows.length < 1 || result.rows[0].requestStatus.trim() !== 'Pending') {
        return res.status(404).json({
          message: 'Cannot update request',//can only update for pending
        });
      }

      client.query('UPDATE request set description = $1, department = $2 , "requestType"= $3, "requestLevel"= $4  where "requestId" = $5 and "userId" = $6', [request.description, req.userData.department, request.requestType, request.requestLevel, req.params.requestId, req.userData.userId], (queryErr, reslt) => {
        done();
        if (queryError) {
          console.log(queryError);
          res.status(400).send(queryError);
